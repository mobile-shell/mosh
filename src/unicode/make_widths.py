...
def make_widths(narrow, wide, delta):
    ...
    deltamap = bytearray(b'=' * unicode_codespace)
    apply_table_to_map(deltamap, east_asian_ambiguous, '2')

    for (charmap, name) in [[basemap, narrow], [eawwidemap, wide], [deltamap, delta]]:
        edit_widths.write_maps(charmap, name)
    return None
...
def main(argv):
    ...
    argparser.add_argument('--delta-table',
                           help='Name for delta (narrow-to-wide) map output file',
                           default='eaw_delta_map')
    opts = argparser.parse_args(argv)

    make_widths(opts.narrow_table, opts.wide_table, opts.delta_table)
    return None
